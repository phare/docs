...
Our monitoring infrastructure uses a vast range of IPs provided by [Bunny.net global network](https://bunnycdn.com/api/system/edgeserverlist) to provide the widest available monitoring offer at an affordable price. Phare is not the only company using these IP addresses. Therefore, IP filtering should not be relied upon to control access to our monitoring traffic.
