...
Region API codes follow the format `<continent>-<country>-<iata>`.
...
Because Phare monitoring agents run on global edge networks, IP addresses are dynamic. Rather than maintaining brittle IP allowlists, Phare supports modern cryptographic bot authentication and header-based filtering.

### Web Bot Auth (Verified Bot)

Phare monitoring agents implement the IETF standard **Web Bot Auth** ([RFC 9421](https://datatracker.ietf.org/doc/html/rfc9421) HTTP Message Signatures).

Every HTTP check executed by Phare includes cryptographic Ed25519 signatures in the request headers (`Signature`, `Signature-Input`, and `Signature-Agent`).

Phare's signature directory is available at:

```
https://phare.io/.well-known/http-message-signatures-directory
```

### User-Agent & Secret token

If your web server or firewall does not support web-auth-bot yet, the next best thing is to filter Phare's traffic with an [User-Agent secret](/uptime/monitors#headers).
...
- **No IP allowlisting**: Monitoring traffic comes from [Bunny.net's shared IP ranges](https://api.bunny.net/mc/nodes/plain), so fixed IPs can't be provided.
